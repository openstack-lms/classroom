...
export const setupSocketHandlers = (io: Server) => {
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('event-create', (data: { classId: string, event: any }) => {
      if (data.classId) {
        io.to(`class-${data.classId}`).emit('event-created', data.event);
      } else {
    ...
    socket.on('event-update', (data: { classId: string, event: any }) => {
      if (data.classId) {
        io.to(`class-${data.classId}`).emit('event-updated', data.event);
      } else {
    ...
    socket.on('event-delete', (data: { classId: string, eventId: string }) => {
      if (data.classId) {
        io.to(`class-${data.classId}`).emit('event-deleted', data.eventId);
      } else {
