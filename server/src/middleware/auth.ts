...
export const createAuthMiddleware = (t: any) => {
  ...
  const isAdminOfInstitution = t.middleware(async ({ next, ctx, input }: MiddlewareContext) => {
    if (!ctx.user) {
      throw new TRPCError({
        code: 'UNAUTHORIZED',
        message: 'Not authenticated',
    ...
    const institutionId = (input as { institutionId: string })?.institutionId;
    if (!institutionId) {
      throw new TRPCError({
        code: 'BAD_REQUEST',
    ...
    const isAdmin = await prisma.institution.findFirst({
      where: {
        id: institutionId,
        admins: {
          some: {
    ...
    if (!isAdmin) {
      throw new TRPCError({
        code: 'FORBIDDEN',
        message: 'Not an admin of this institution',
      });
    }
  ...
  const isTeacherOfInstitution = t.middleware(async ({ next, ctx, input }: MiddlewareContext) => {
    if (!ctx.user) {
      throw new TRPCError({
        code: 'UNAUTHORIZED',
        message: 'Not authenticated',
    ...
    const isTeacher = await prisma.institution.findFirst({
      where: {
        id: institutionId,
      }
    });
    ...
    if (!isTeacher) {
      throw new TRPCError({
        code: 'FORBIDDEN',
        message: 'Not a teacher of this institution',
      });
    }
  ...
  const isStudentOfInstitution = t.middleware(async ({ next, ctx, input }: MiddlewareContext) => {
    if (!ctx.user) {
      throw new TRPCError({
        code: 'UNAUTHORIZED',
        message: 'Not authenticated',
    ...
    const isStudent = await prisma.institution.findFirst({
      where: {
        id: institutionId,
      }
    });
    ...
    if (!isStudent) {
      throw new TRPCError({
        code: 'FORBIDDEN',
        message: 'Not a student of this institution',
      });
    }
